...
const DataKantorHRD: React.FC = () => {
  ...
  const fetchOffices = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${API_URL}/hrd/offices`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (!res.ok) throw new Error('Gagal memuat data');
      const data = await res.json();
      const list = (data.offices || []).map((o: any) => ({
        ...o,
        name: o.name || 'Kantor',
        address: o.address || '-',
        latitude: Number(o.latitude) || 0,
        longitude: Number(o.longitude) || 0,
        radius_meters: o.radius_meters ? Number(o.radius_meters) : 200
      }));
      setOffices(list);
    } catch (err) {
      console.error(err);
      alert('Terjadi kesalahan saat memuat data kantor');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (o: Office) => {
    setEditingId(o.id);
    setFormData({ 
      name: o.name || '', 
      address: o.address || '', 
      radius_meters: o.radius_meters || 200 
    });
    setFormPosition(new L.LatLng(o.latitude, o.longitude));
    setIsFormModalOpen(true);
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/hrd/offices/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingId ? `${API_URL}/hrd/offices/${editingId}` : `${API_URL}/hrd/offices`;
  ...
  return (
    <div className="p-4 md:p-8 flex-1 bg-gray-50 min-h-screen">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Data Lokasi Kantor</h1>
        <p className="text-gray-500 text-sm mt-1">Kelola data kantor beserta lokasinya untuk absensi</p>
      </div>

      <div className="bg-white rounded-lg border border-gray-200 p-4 mb-6 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex-1 w-full max-w-2xl">
            <label className="block text-sm font-medium text-gray-700 mb-1">Cari Kantor Atau Tempat</label>
                  ...
                  <tr key={office.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <div className="text-sm font-bold text-gray-900">{office.name}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200">
                        {office.radius_meters || 200} m
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col">
                        <span className="text-xs text-gray-400 mb-1">{office.latitude.toFixed(5)}, {office.longitude.toFixed(5)}</span>
                        <button 
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="overflow-y-auto p-5 flex flex-col gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">Radius toleransi presensi karyawan dari titik lokasi kantor (default: 200m).</p>
              </div>
              <div>
                ...
                <div className="flex gap-2 mb-2">
                  <input 
                    type="text"
                    placeholder="Contoh: Gedung Sudirman..."
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col">
              ...
              <div className="h-[400px] w-full bg-gray-100">
                ...
                >
                  <TileLayer
                    ...
                    url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                  />
                  <Marker position={[selectedOffice.latitude, selectedOffice.longitude]}>
                    <Popup>
                  ...
                  <Circle 
                    center={[selectedOffice.latitude, selectedOffice.longitude]} 
                    radius={selectedOffice.radius_meters || 200}
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row justify-between sm:items-center gap-2">
              <p className="text-sm text-gray-700">{selectedOffice.address}</p>
              <span className="text-xs font-semibold px-2.5 py-1 bg-blue-100 text-blue-800 rounded-full shrink-0">
                Radius Geofence: {selectedOffice.radius_meters || 200} m
              </span>
            </div>
          </div>
        </div>
